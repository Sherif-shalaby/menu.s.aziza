...
<tr class="row_{{ $row_id }}" data-row_id="{{ $row_id }}">
    ...
    <td style = 'width: 15%'>
        <div class="input-group my-group ">
            {!! Form::select('sizes[' . $row_id . '][size_id]', $sizes,!empty($item) ? $item->size_id : false,['class' => 'form-control select2','style' => 'width: 60%','data-live-search' => 'true', 'placeholder' => __('lang.size')]) !!}
            <span class="input-group-btn">
                @can('settings.size.create')
                    <button class="btn-modal btn btn-default bg-white btn-flat"
                        data-href="{{ action('Admin\SizeController@create') }}?quick_add=1"
