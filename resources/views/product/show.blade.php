...
@section('content')
    ...
    <div class="container mx-auto mt-14">
        <div class="flex flex-row flex-wrap min-h-full h-96">
            ...
            <div class=" product-slider xs:w-full lg:w-1/2">
                            @foreach ($product->getMedia('product') as $image)
                                <img src="@if (!empty($image->getUrl())) {{ $image->getUrl() }}@else{{ asset('uploads/' . session('logo')) }} @endif"
                                    class="m-4 aspect-square" alt="" width="300px">
                            @endforeach
             </div>
            ...
            <!-- <div class="flex-1 xs:w-full lg:w-1/2 px-16 @if ($product->getMedia('product')->count() == 0) md:block xs:hidden @endif">
                ...
                <div class="flex flex-row items-center  w-full  @if ($product->getMedia('product')->count() == 0) xs:hidden @endif">
                    ...
                    <div class="flex-1 w-full ">
                        <div class="w-full product-slider">
                            @foreach ($product->getMedia('product') as $image)
                                <img src="@if (!empty($image->getUrl())) {{ $image->getUrl() }}@else{{ asset('uploads/' . session('logo')) }} @endif"
                                    class="w-full aspect-square" alt="" width="300px">
                            @endforeach
                        </div>
                    </div>
        ...
        -->
            <div class="flex-1 xs:w-full lg:w-1/2 text-dark">
                <div class="flex flex-col px-16 py-8 bg12">
                    <div class="flex-1">
        ...
        <!-- <div class="flex">
            ...
            <div class="flex-1 mt-2 ml-1 mr-1 text-center cart_items_page">
                <a href="{{ action('CartController@view') }}" class="text-center ">
                    <button class="relative w-full font-semibold text-white bg10 rounded-xl flex-c-m"
                        style="    height: 50px;
            padding: 5px;
            width: 100%;
