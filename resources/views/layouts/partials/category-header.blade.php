<div class="w-full flex flex-row bg-no-repeat bg-center bg-cover text-center items-center"
    style="height: 200px; background-image: url('@if(!empty(session('breadcrumb_background_image'))){{ asset('uploads/' . session('breadcrumb_background_image')) }}@else{{ images_asset('images/default-breadcrumb-bg.png',false) }}@endif');">
    <div class="w-full">
